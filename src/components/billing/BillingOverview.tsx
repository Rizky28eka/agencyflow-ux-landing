...
import { LineChart, CartesianGrid, XAxis, YAxis, Line, ResponsiveContainer, Area } from 'recharts';
...
export const BillingOverview = ({ currentPlan, nextInvoice, paymentMethod }: BillingOverviewProps) => {
  ...
  const daysUntilRenewal = Math.ceil(
    (new Date(currentPlan.renewalDate).getTime() - new Date().getTime()) /
      (1000 * 60 * 60 * 24)
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-r from-blue-50 via-white to-purple-50 border-primary/20">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge
              variant={getStatusVariant(currentPlan.status)}
